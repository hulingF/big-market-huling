...
import com.huling.domain.activity.event.ActivitySkuStockZeroMessageEvent;
...
import com.huling.domain.activity.model.vo.UserRaffleOrderStateVO;
import com.huling.infrastructure.dao.*;
import com.huling.infrastructure.dao.po.*;
    ...
    public void doSaveOrder(CreateOrderAggregate createOrderAggregate) {
        try {
            ...
            transactionTemplate.execute(status -> {
                try {
                    // 1. 写入订单【幂等uq_out_business_no】
                    raffleActivityOrderDao.insert(raffleActivityOrder);
                    // 2. 更新账户
                    int count = raffleActivityAccountDao.updateAccountQuota(raffleActivityAccount);
                    // 3. 创建账户【更新为0，则账户不存在，创新新账户，幂等uq_user_id_activity_id】
                    if (0 == count) {
